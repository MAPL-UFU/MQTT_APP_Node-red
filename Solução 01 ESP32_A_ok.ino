...
void reconnectMQTT() {
  while (!client.connected()) {
    ...
    if (client.connect("Esp32ValvulaA")) {
      ...
      const char* msgvalA_status = "ESP32A Connectado";
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (command == 1) {
    ...
    publishSensorState(digitalRead(S1));  // Publica o estado do sensor fim de curso S1
  } else if (command == 0) {
    closeValve();  // Recua o atuador
    publishSensorState(digitalRead(S0));  // Publica o estado do sensor fim de curso S0
  }
}
